...
export default async function ManageBookingPage({ searchParams }: { searchParams: Promise<{ filter?: string }> }) {
  ...
  try {
    let query = `SELECT * FROM bookings WHERE working_status = 'Reject' ORDER BY created_at DESC`;
    if (filter !== "All") {
      query = `SELECT * FROM bookings WHERE type = '${filter}' AND working_status = 'Reject' ORDER BY created_at DESC`;
    }
  ...
  return (
    <div className="p-8 font-sans bg-white min-h-screen text-[13px]">
      ...
      {/* Title */}
      <h1 className="text-[22px] font-bold text-gray-800 tracking-tight mb-4">Rejected Bookings</h1>
