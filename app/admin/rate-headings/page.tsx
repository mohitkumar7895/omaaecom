...
import { deleteRateHeading } from "../../actions/rateHeadings";

export const dynamic = 'force-dynamic';

export default async function RateHeadingsPage() {
  let rateHeadings = [];

  try {
    const [rows]: any = await pool.query("SELECT * FROM rate_headings ORDER BY id ASC");
    rateHeadings = rows;
  } catch (error) {
    console.error("Error fetching rate headings:", error);
  }
    ...
    <div className="flex-1 bg-gray-50 min-h-screen">
      <div className="p-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-[26px] font-bold text-gray-800 tracking-tight">Rate Heading</h1>
          <Link 
            href="/admin/rate-headings/new" 
          ...
          >
            ...
            <span>Add Rate Heading</span>
          </Link>
        </div>

        {/* Action Bar */}
        <div className="flex justify-between items-end mb-4">
          <ExportButtons tableId="rate-headings-table" />
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
          <div className="overflow-x-auto">
            <table id="rate-headings-table" className="w-full text-left border-collapse whitespace-nowrap">
              <thead>
                <tr className="bg-[#2c3e50] text-white text-[13px]">
                  <th className="px-4 py-3.5 font-semibold w-24 border-r border-gray-600">ID <span className="text-[10px] ml-1 text-gray-400">▲▼</span></th>
              ...
              <tbody>
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center justify-center space-x-2">
                        <Link 
                          href={`/admin/rate-headings/edit/${row.id}`}
                ...
                {rateHeadings.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-4 py-8 text-center text-gray-500">
                      No rate headings found.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="bg-white border-t border-gray-100 px-4 py-3 flex items-center justify-between text-sm text-gray-600">
            <div>Showing 1 to {rateHeadings.length} of {rateHeadings.length} entries</div>
